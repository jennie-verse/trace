...
변경 내용: `webapp-data`의 `events/` 폴더를 읽는 파서 추가, Service Worker 캐시 `trace-shell-v9` → `trace-shell-v10`.

검토 방식: jsdom으로 `index.html` + `src/app.js`를 실제로 실행하고, `shared/v1/sync.js`를 가짜 모듈로 바꿔 네트워크 없이 여러 데이터 상황을 재현했습니다. 검사 35건 전부 통과했습니다(atlas와 공용 스크립트).
...
| `events/`에 `.gitkeep`만 있을 때 | 콘솔 오류 0건. 타임라인이 비어 있음 |
| 파일 이름 필터 | `<앱>.<기기>.<YYYY-MM>.json` 이외의 파일과 하위 폴더는 내려받지 않음 |
| 3개월 범위 제한 | 그보다 오래된 달의 파일을 읽지 않음 |
| 그날 항목만 표시 | 이벤트의 `at`을 로컬 날짜로 바꿔 선택한 날짜와 비교 |
| 시간순 정렬 | tide·clip·events가 섞여도 시각 순서대로 정렬됨 |
| tombstone | 같은 `id`에 `"deleted": true`가 뒤에 붙으면 타임라인에서 빠짐 |
| 모르는 스키마 버전 | `v: 2` 레코드를 조용히 건너뜀 |
| 뱃지 표기 | `FOCUS`, `LOOM`처럼 원래 앱 이름을 대문자로 표시 |
| 캐시 호환 | 기존 version 2 캐시에 `events` 키가 없어도 빈 배열로 시작, 오류 없음 |
| 기존 clip 데이터 회귀 | 기존 clip 항목이 타임라인에 그대로 표시됨 |

## 2. 보안 관련해 확인한 항목

| 항목 | 결과 |
|---|---|
| 사용자 텍스트의 HTML 실행 | `detail`에 `<img src=x onerror=...>`를 넣어도 타임라인 DOM에 태그로 들어가지 않음. 전 구간 `textContent` |
| 외부 요청 | 추가된 요청 없음. 여전히 `api.github.com`만 사용 |

## 3. 실기기에서 직접 확인해야 할 항목 (Pending)

- [ ] 실제 `webapp-data`에 events 파일이 생긴 뒤(2단계 focus 연결 이후) 타임라인에 실제로 섞여 나오는지
- [ ] 3개월보다 오래된 날짜로 이동했을 때 events가 비어 보이는 것이 실사용에서 불편한지 (불편하면 그 달만 따로 읽는 방식으로 보완)
- [ ] Service Worker가 `trace-shell-v10`으로 갱신된 뒤 다른 앱의 캐시가 남아 있는지
